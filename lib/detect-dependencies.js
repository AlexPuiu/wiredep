...
function findMainFiles(config, component, componentConfigFile) {
  ...
  if ($._.isString(componentConfigFile.main)) {
	...
	var skipMinification = $._.toArray(config.get('skipMinification'));
	
    if(config.get('overrideMainWithMinified') === true && !$._.contains(skipMinification, component.toString())) {
        ...
        for (var curCount = 0, len = filePaths.length; curCount < len; curCount++) {
				...
				if(filePath.indexOf('min.css') > -1 || filePath.indexOf('min.js') > -1) {
					continue;
				}
				if(filePath.indexOf('.css') > -1) {
					var filePathMin = filePath.replace('.css','.min.css');
				...
				if(filePath.indexOf('.js') > -1) {
					var filePathMin = filePath.replace('.js','.min.js');
					file = $.path.join(config.get('bower-directory'), component, filePathMin);
					 if ($.fs.existsSync(file)) {
						//console.log('###' + file +'\n');
						filePaths[curCount] = filePathMin;
